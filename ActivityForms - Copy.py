...
def main_activityforms() -> None:
    ...
    BUFFER = 5                                             # To be defined based on the maximum timeframe within which order details can be modified in the Bizom System
    JUMP_SIZE = 100
    STATUSES = []
    STATUS = []
    TIME_OUT_MAX = 45
    ...
    for each in daterange(START, END):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            while True:
                ...
                if not terminater(jason=jason, record_path='Activityformdatas',loop_timeout=None):
                    logger.debug("API call failed to respond with any data, iterating to the next date")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    output_df = json_to_df(jason, rec_path='Activityformdatas')                    
