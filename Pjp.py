    ...
    for each in daterange(START, END):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            while True:
                ...
                if not terminater(jason=jason, record_path='Pjps',loop_timeout=None):
                    logger.debug("Internal Break criteria achieved. Breaking the batch loop")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    output_df = json_to_df(jason, rec_path='Pjps')         
