...
from shared_modules.ops_utilities import check_directory_exists, terminater, add_inj_date, check_update, merge_in_path, check_directory_exists 
    ...
    for each in daterange(START, END):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            while True:
                ...
                if not terminater(jason=jason, record_path='Orders',loop_timeout=None):
                    logger.debug("API call failed to respond with any data, iterating to the next date")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    jason = find_json_objects(response, json_obj_name='Orders')
                    
                    # Converting JSON object into a dataframe object
                        # Order Level Data 
                    try:
                        output_df = dataframarize(jason['order'])
                    except ValueError:
                        output_df = dataframarize([jason['order']])

                    try:
                        output_df2 = dataframarize([subeach for each in list(output_df['order_details']) for subeach in each if isinstance(subeach, dict)])
                    except ValueError:
                        raise
