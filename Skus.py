...
from shared_modules.datetime_utilities import dt_to_string, step_daterange, string_to_dt
...
# def main():
    ...
    # Configure other global Variables
    URL_EXT = '/skunits/getallcompanyskus'
    START =  string_to_dt(DEFAULT_ST_DT).date()                  # Needs to be set up to read from he setting.json file eventually
    ...
    for each in step_daterange(start_date = START, end_date = END, step_size=7):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            BREAK_END = string_to_dt(each[-1]).date() - td(days=1)
            REDO = False
            ...
            while True:
                ...
                PARAMS['fromdate'] = dt_to_string(each[0])
                PARAMS['todate'] = dt_to_string(each[-1])
                ...
                logger.debug("Json objects reieved and located")

                if not terminater(jason=jason, record_path='Products',loop_timeout=None):
                    logger.debug("API call failed to respond with any data, iterating to the next date")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    output_df = json_to_df(jason, rec_path='Products')                    
                    ...
                    file_path = os.path.join(INT_STAGE_PATH, f'STAGE_{NAME}_{PARAMS["fromdate"]}_{PARAMS["todate"]}_{PARAMS["startseq"]}_{PARAMS["endseq"]}.csv')
    ...
    if REDO:
        ...
        BREAK_END = string_to_dt(each[-1]).date()
