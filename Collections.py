...
def main_collections() -> None:
    ...
    URL_EXT = '/collections/getcollections/'
    ...
    for each in daterange(START, END):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            while True:
                ...
                fromdate = dt_to_string(each)
                todate = dt_to_string(each)
                startseq = str(START_SEQ)
                endseq = str(END_SEQ)

                # Create URL for call
                url = create_url(BASE_URL=BASE_URL,URL_EXT=URL_EXT,args=[fromdate,todate,startseq,endseq])
                ...
                if not terminater(jason=jason, record_path='Collections',loop_timeout=None):
                    logger.debug("API call failed to respond with any data, iterating to the next date")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    output_df = json_to_df(jason, rec_path='Collections')                   
                    ...
                    try:
                        file_path = os.path.join(INT_STAGE_PATH, f'STAGE_{NAME}_{fromdate}_{startseq}_{endseq}.csv')
