...
from shared_modules.datetime_utilities import dt_to_string, string_to_dt, step_daterange
...
def main_salesreturn() -> None:
    ...
    URL_EXT = '/salesreturns/getSalemansalesreturn/'
    ...
    for each in step_daterange(start_date = START, end_date = END, step_size=7):
        ...
        if time.time() > LOOP_TIMEOUT:
            ...
            while True:
                ...
                if not terminater(jason=jason, record_path='Salemansalesreturns',loop_timeout=None):
                    logger.debug("API call failed to respond with any data, iterating to the next date")
                    break
                else:
                    
                    # Save the relevant JSON object in response       
                    output_df = json_to_df(jason, rec_path='Salemansalesreturns')                    
